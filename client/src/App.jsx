...
// --- 🤖 GESTION DU NARRATEUR (Mode Hélium) ---
const speak = (text) => {
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance(text);
    utterance.lang = 'fr-FR';
    
    // Essayer de forcer une voix Google si possible
...
// Fonction utilitaire pour reconstruire la phrase complète (Compatible Pick 2 & 3)
const buildFullSentence = (blackText, whiteTexts) => {
    let sentence = blackText;
    // On remplace séquentiellement les trous (____) par les réponses
    whiteTexts.forEach(answer => {
        let cleanAnswer = answer.replace(/\.$/, ""); 
        // Regex : cherche 3 underscores ou plus (____)
        sentence = sentence.replace(/_{3,}/, ` ${cleanAnswer} `);
    });
    
    // Fallback : Si aucun remplacement n'a eu lieu (ex: carte sans trou), on concatène
...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on("round_winner", (info) => {
        ...
        setTimeout(() => {
            const fullSentence = buildFullSentence(blackCard.text, info.winningCards);
            speak(`${info.winnerName} a gagné ! ${fullSentence}`);
        }, 500);
    });

    socket.on("game_over", (data) => { setGrandWinner(data); setWinnerInfo(null); playSound('win.mp3'); speak(`Victoire finale de ${data.winnerName} !`); });
  ...
  const triggerNextRound = () => {
      socket.emit('trigger_next_round', roomCode);
  };
  ...
  const toggleSelection = (textToSend, original) => {
      ...
      if (existingIndex > -1) {
          ...
          if (selectedCards.length < pickNeeded) {
              ...
              setSelectedCards(newSelection);
              if (pickNeeded === 1) { confirmPlay(newSelection); }
          }
      }
  };
  ...
  if (winnerInfo) { 
      return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-10 relative">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-yellow-400 mb-8 drop-shadow-lg text-center">
              🏆 {winnerInfo.winnerName} GAGNE !
          </h1>
          
          {/* AFFICHAGE PHRASE COMPLÈTE CORRIGÉ POUR PICK 2 */}
          <div className="bg-white text-black p-8 rounded-2xl shadow-2xl max-w-3xl transform rotate-1 transition hover:scale-105 border-4 border-black mb-12">
              <p className="text-2xl md:text-3xl font-black text-center leading-relaxed">
                  {blackCard.text.split(/_{3,}/).map((part, index, array) => (
                      <span key={index}>
                          {part}
                          {index < array.length - 1 && winnerInfo.winningCards[index] && (
                             <span className="text-purple-700 underline decoration-4 decoration-yellow-400 mx-2">
                                 {winnerInfo.winningCards[index].replace(/\.$/, "")}
                             </span>
                          )}
                      </span>
                  ))}
          ...
          {amIHost ? (
              <button onClick={triggerNextRound} className="animate-bounce bg-green-500 hover:bg-green-600 text-white font-black py-4 px-8 rounded-full shadow-[0_0_20px_rgba(34,197,94,0.6)] text-2xl transition transform hover:scale-110 border-4 border-white">
                  MANCHE SUIVANTE ➡️
              </button>
          ) : (
              <div className="text-gray-400 animate-pulse text-lg text-center">
  ...
  if (gameStarted) {
    ...
    if (!amIJudge && pickAmount > 1) {
        if (selectedCards.length === 0) { instructionText = "1️⃣ Choisis ta PREMIÈRE carte"; instructionClass = "text-yellow-400 font-bold animate-pulse"; }
        else if (selectedCards.length < pickAmount) { instructionText = "2️⃣ Choisis ta DEUXIÈME carte"; instructionClass = "text-blue-400 font-bold animate-pulse"; }
        else { instructionText = "✅ Tout est bon ? Valide !"; instructionClass = "text-green-400 font-bold"; }
    }
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-4 relative">
        ...
        <div className="w-full flex justify-between items-start mb-2 px-4 pt-4">
          ...
          <div className="flex flex-col items-end bg-gray-800 p-3 rounded-lg shadow-md border border-gray-700">
              {players.map(p => (
                  <div key={p.id} className={`flex items-center justify-end gap-3 text-sm mb-2 last:mb-0 ${p.id === judgeId ? 'text-yellow-400 font-bold' : 'text-white'}`}>
                      ...
                      <div className="text-right leading-tight">
                          <div className="font-semibold">{p.username} {p.id === judgeId && "👑"}</div>
                          <div className="text-xs opacity-70">{p.score}/{roomSettings.scoreLimit} pts</div>
                      </div>
                      <img src={`https://api.dicebear.com/7.x/fun-emoji/svg?seed=${p.username}`} alt="Avatar" className="w-10 h-10 rounded-full bg-gray-200 border-2 border-white/20 shadow-sm" />
                  </div>
              ))}
          </div>
        </div>
                ...
                <>
                    <div className="flex flex-col items-center px-4 mb-2">
                        ...
                        {pickAmount > 1 && selectedCards.length === pickAmount && (
                            <button onClick={() => confirmPlay()} className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-8 rounded-full shadow-lg animate-pulse mb-2">ENVOYER LES RÉPONSES 🚀</button>
                        )}
                    </div>
                    ...
                    <div className="w-full relative z-10">
                        <div className="flex overflow-x-auto gap-4 px-4 pb-6 pt-16 items-end w-full touch-pan-x snap-x">
                            {myHand.map((card, index) => {
                                ...
                                return (
                                <div key={index} onClick={() => handleCardClick(card)} className={`snap-center flex-shrink-0 p-4 rounded-lg w-48 h-64 shadow-lg cursor-pointer transition transform border-4 relative ${card === JOKER_TEXT ? 'bg-purple-600 text-white' : 'bg-white text-gray-900'} ${isSelected ? 'border-blue-500 -translate-y-12' : 'border-transparent hover:-translate-y-4 hover:border-purple-300'}`}>
  ...
  if (isInRoom) { 
      ...
      <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-10">
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md border border-gray-700">
              {amIHost ? (<div className="mb-6 bg-gray-700 p-4 rounded border border-gray-600"><h3 className="font-bold text-yellow-400 mb-3 border-b border-gray-600 pb-1">⚙️ Paramètres</h3><div className="flex justify-between items-center mb-3"><label>Points pour gagner :</label><input type="number" value={roomSettings.scoreLimit} onChange={(e) => updateSettings('scoreLimit', e.target.value)} className="w-16 bg-gray-900 border border-gray-500 rounded px-2 py-1 text-center font-bold" min="1" max="50" /></div><div className="flex justify-between items-center"><label>Temps par tour (sec) :</label><input type="number" value={roomSettings.timerDuration} onChange={(e) => updateSettings('timerDuration', e.target.value)} className="w-16 bg-gray-900 border border-gray-500 rounded px-2 py-1 text-center font-bold" min="0" max="120" placeholder="0 = infini"/></div></div>) : (<div className="mb-6 text-center text-sm text-gray-400 bg-gray-700 p-2 rounded"><p>🎯 Objectif : {roomSettings.scoreLimit} points</p><p>⏱️ Temps : {roomSettings.timerDuration > 0 ? roomSettings.timerDuration + 's' : 'Infini'}</p></div>)}
